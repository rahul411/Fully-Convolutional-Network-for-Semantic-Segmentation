...
delta = 63.0/255.0
TRAIN_FLAG = True
model_path = 'modelpspnet_with_focalLoss'
...
def focal_loss(prediction_tensor, target_tensor, weights=None, alpha=0.25, gamma=2):
    ...
    sigmoid_p = tf.sigmoid(prediction_tensor)
...
with graph.as_default():
    ...
    loss = focal_loss(logits,tf.one_hot(gt,num_classes))
...
with tf.Session(graph=graph) as sess:
    ...
    writer = tf.summary.FileWriter("output_focalLoss", sess.graph)
    restore_vars = [
