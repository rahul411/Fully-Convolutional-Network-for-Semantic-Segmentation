...
def fc_layer(bottom, name):
    with tf.variable_scope(name) as scope:
        ...
        if name == 'fc6':
        	weight, conv_biases = get_var_fc_layer(name,[7,7,512,4096])
        else:
        	weight, conv_biases = get_var_fc_layer(name,[1,1,4096,4096])
...
def get_var_fc_layer(name, shape):
...
with tf.Session(graph=graph) as sess:
    ...
    for v in tf.global_variables():
        var += np.prod(v.get_shape().as_list())
    print(var)
    ...
    for epoch in range(n_epochs):
        ...
        while True:
            try:
                ...
                _, steploss = sess.run([optimizer, loss], feed_dict= {
                        x: train_images,
                        y: train_labels
                    })
