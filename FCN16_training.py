...
def model(img):
    ...
    upscore_32 = upscore_layer(s_layer_32, 'upscore_s_layer_32', tf.shape(pool4), num_classes, ksize=4, stride=2)

    fuse_layer = tf.add(s_layer_16, upscore_32)

    upscore_16 = upscore_layer(fuse_layer,'upscore_fuse_layer', tf.shape(bgr), num_classes, ksize=32, stride=16)

    pred = tf.argmax(upscore_16,axis=3)

    return pred, tf.nn.softmax(upscore_16)
